...
public class TestFeatureStructure extends BaseStructure {
    ...
    public TestFeatureStructure() {
        super("TEST_STRUCTURE");
    }

    @Override
    protected void generateSteps(Vector3Int vector3Int, WritableWorld writableWorld, Random random, Rotation rotation) {
        BlockState goldBlock = platform.getWorldHandle().createBlockState("minecraft:gold_ore");
        BlockState goldBlockDeepslate = platform.getWorldHandle().createBlockState("minecraft:deepslate_gold_ore");
        ...
        GeometryUtil.cube(
                vector3Int,
                size,
                action -> writableWorld.setBlockState(action, randomOreType ? goldBlock : goldBlockDeepslate)
        );
    }
}
