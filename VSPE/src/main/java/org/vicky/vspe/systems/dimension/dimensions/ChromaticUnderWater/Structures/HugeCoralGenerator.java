...
import org.vicky.vspe.systems.dimension.StructureUtils.Generators.ProceduralCrystalShardGenerator;
...
public class HugeCoralGenerator extends BaseStructure {
    ...
    // --- TREE-LIKE CORAL ---
    private static void generateTreeCoral(Random random, WritableWorld world, Vector3Int origin, int heighto, int span, BlockState blueCoral, BlockState coralFan, Platform platform) {
        new ProceduralCrystalShardGenerator.Builder()
            .palette(List.of(
                    platform.getWorldHandle().createBlockState(Material.GREEN_STAINED_GLASS.getKey().asString()),
                    platform.getWorldHandle().createBlockState(Material.YELLOW_STAINED_GLASS.getKey().asString()),
                    platform.getWorldHandle().createBlockState(Material.ORANGE_STAINED_GLASS.getKey().asString()),
                    platform.getWorldHandle().createBlockState(Material.RED_STAINED_GLASS.getKey().asString())
            ))
            .size(
                    heighto,
                    7
            )
            .glow(true, true, 0.9f, platform)
            .hollow(false)
            .tipPercentage(0.3)
            ...
            .build()
            .generate(random, world, origin.mutable().add(0, -10, 0), platform);
        /*
        new ProceduralTubeGenerator.Builder()
            ...
            .rimCap(false)
            .materials(blueCoral, platform.getWorldHandle().createBlockState(Material.WATER.getKey().asString()))
            .build()
        ...
        */
        ...
        new ProceduralMushroomGenerator.Builder()
            ...
            .stemTaperPct(0.4f)
            .stemTilt(0.24f, 15.0)
            .ridgeMat(platform.getWorldHandle().createBlockState(Material.QUARTZ_BLOCK.getKey().asString()))
            .spots(
                    platform.getWorldHandle().createBlockState(Material.RED_MUSHROOM_BLOCK.getKey().asString()),
                    0.25f
            )
            .materials(
                    platform.getWorldHandle().createBlockState(Material.MUSHROOM_STEM.getKey().asString()),
                    platform.getWorldHandle().createBlockState(Material.WHITE_CONCRETE.getKey().asString())
            )
            .hollowCap()
            .build()
            .generate(random, world, origin);
        ...
        for (int i = 0; i < branchCount; i++) {
            ...
            for (int j = 0; j < (int) branchLen; j++) {
                ...
                if (random.nextFloat() < 0.3) {
                    world.setBlockState(Vector3Int.of(origin, bx, y + 1, bz), coralFan);
                }
            }
        }
         */
            ...
            if (y > height / 3 && random.nextInt(4) == 0) {
                ...
                for (int b = 0; b < branches; b++) {
                    ...
                    if (random.nextBoolean()) {
                        world.setBlockState(Vector3Int.of(origin, bx, y + bl, bz), coralFan);
                    }
                }
            }
        }
        ...
        for (int i = 0; i < rings; i++) {
            ...
            for (int phiStep = 0; phiStep < 360; phiStep += 15) {
                ...
                if (random.nextInt(5) == 0) {
                    world.setBlockState(Vector3Int.of(origin, ix, iy + 1, iz), coralFan);
                }
            }
        }
    }

    private static void placeThickCoralPath(WritableWorld world, Vector3Int origin, int x, int y, int z, int thickness, BlockState state) {
        for (int dx = -thickness; dx <= thickness; dx++) {
            for (int dz = -thickness; dz <= thickness; dz++) {
                if (dx * dx + dz * dz <= thickness * thickness) {
                    world.setBlockState(Vector3Int.of(origin, x + dx, y, z + dz), state);
                }
            }
        }
    }
    ...
    protected void generateSteps(Vector3Int location, WritableWorld world, Random random, Rotation var4) {
        ...
        switch (/*coralType*/1) {
            ...
            case 1 -> generateTreeCoral(random, world, location, height, span, coral, coralFan, this.platform);
