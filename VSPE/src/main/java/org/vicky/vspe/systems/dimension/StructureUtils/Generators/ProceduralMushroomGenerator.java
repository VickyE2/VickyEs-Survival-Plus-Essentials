 ...
 */
public class ProceduralMushroomGenerator extends ProceduralStructureGenerator {
    ...
    private final BlockState stemMaterial, capMaterial, spotMaterial, ridgeMaterial;
    ...
    public ProceduralMushroomGenerator(Builder b) {
    ...
    private void placeStem(Vector3Int origin) {
        ...
        /*
        VSPE.getInstancedLogger().info("📍  Placing stem at " +
        ...
        for (int i = 0; i < stemHeight; i++) {
            ...
            if (rnd.nextFloat() < stemTiltChance) {
                /*
                VSPE.getInstancedLogger().info("Producing yaw twist...");
                VSPE.getInstancedLogger().info(String.format(
                        "pre dir=(%.2f,%.2f,%.2f)",
                        dir.x, dir.y, dir.z
                ));
                 */
            ...
            if (rnd.nextFloat() < stemTiltChance * 0.5) {  // half chance for pitch
                /*
                VSPE.getInstancedLogger().info("Producing pitch twist...");
                 */
        ...
        // final cap placement
        placeCap(Vector3Int.of(finalX, finalY, finalZ));
    }


    private void placeCap(Vector3Int origin) {
        /*
        VSPE.getInstancedLogger().info(String.format("Placing at %s %s %s", origin.getX(), origin.getY(), origin.getZ()));
         */
        ...
        Vector3Int capBase = Vector3Int.of(
                origin.getX(),
    ...
    public interface CapEquation {
         ...
         */
        double getHeight(double xNorm,double yNorm, double height, Random rnd);
    }
    ...
    public static class Builder {
        ...
        BlockState stemMaterial, capMaterial, spotMaterial, ridgeMaterial;
        ...
        public Builder stemTaperPct(float p) {
            this.stemTaperMinPct = clamp(p, 0, 1);
            return this;
        }
        /** chance each block that the stem will kink by up to ±maxTiltDeg */
        public Builder stemTilt(float chance, double maxTiltDeg) {
            this.stemTiltChance   = clamp(chance, 0, 1);
            this.stemMaxTiltDeg   = maxTiltDeg;
            return this;
        }
        public Builder capRadiusRange(int min,int max){capRadiusMin=min;capRadiusMax=max;return this;}
        public Builder capHeightRange(int min,int max){capHeightMin = min; capHeightMax = max;return this;}
        public Builder stemHeightRange(int min,int max, int r){stemRadius = r;stemHeightMin = min; stemHeightMax = max;return this;}
        public Builder materials(BlockState stem, BlockState cap){this.stemMaterial=stem;this.capMaterial=cap;return this;}
        public Builder spots(BlockState spot, float frequency){this.hasSpots=true;this.spotMaterial=spot;this.spotFrequency=frequency;return this;}
        public Builder hollowCap(){this.hollowCap=true;return this;}
        public Builder ridgeMat(BlockState m){this.ridgeMaterial=m;return this;}
        public Builder capEq(CapEquation eq){this.capEquation=eq;return this;}
        public ProceduralMushroomGenerator build(){return new ProceduralMushroomGenerator(this);}
    }
}
