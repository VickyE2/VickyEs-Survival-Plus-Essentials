...
public class AdvancementManager {
   ...
   public final eu.endercentral.crazy_advancements.manager.AdvancementManager ADVANCEMENT_MANAGER = new eu.endercentral.crazy_advancements.manager.AdvancementManager(
      new NameKey("vspe", "manager"), new Player[0]
   );

   public void processAdvancements() {
      VSPE.getInstancedLogger().info(ANSIColor.colorize("yellow[Starting advancement Processing...]"));
      int index = 0;
      Reflections reflections = new Reflections("org.vicky.vspe.features.AdvancementPlus.Advancements", new Scanner[0]);
   ...
   public void saveManagerProgress() {
      for (OfflinePlayer player : Bukkit.getOfflinePlayers()) {
         this.ADVANCEMENT_MANAGER.saveProgress(player.getUniqueId(), new Advancement[0]);
      }
   }
   ...
   public void saveAndUnloadManagerProgress() {
      for (OfflinePlayer player : Bukkit.getOfflinePlayers()) {
         this.ADVANCEMENT_MANAGER.saveProgress(player.getUniqueId(), new Advancement[0]);
         this.ADVANCEMENT_MANAGER.unloadProgress(player.getUniqueId(), new Advancement[0]);
         this.ADVANCEMENT_MANAGER.loadProgress(player.getUniqueId(), new Advancement[0]);
      }
   }
