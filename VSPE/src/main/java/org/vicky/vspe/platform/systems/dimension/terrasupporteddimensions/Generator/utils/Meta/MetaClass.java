    ...
    public MetaMap getMappingFor(String map) {
        ...
        if (this.customValues.containsKey(map)) {
            ...
            if (closestMatch != null) {
                VSPEPlatformPlugin.platformLogger().warn("Failed to get meta mapping for map: " + map + ". Did you mean: " + closestMatch + "?");
                return null;
            } else {
                VSPEPlatformPlugin.platformLogger().warn("Failed to get meta mapping for map: " + map);
                return null;
            }
        }
    }
