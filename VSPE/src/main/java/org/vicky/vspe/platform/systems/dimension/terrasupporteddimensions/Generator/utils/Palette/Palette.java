...
public class Palette implements BasePalette, Ymlable {
    ...
    public void addLayer(Map<PlatformMaterial, Integer> materials, int layerThickness) {
        ...
        for (Entry<PlatformMaterial, Integer> material : materials.entrySet()) {
    ...
    public void addLayer(Map<PlatformMaterial, Integer> materials, int layerThickness, StringBuilder sampler) {
        Map<ResourceLocation, Integer> instance = new HashMap<>();
        ...
        if (!this.layers.isEmpty() || !this.sampledLayers.isEmpty()) {
            for (Entry<Object, Integer> entry : this.layers.entrySet()) {
                ...
                if (var6 instanceof Map<?, ?> key) {
                    ...
                    for (Entry<?, ?> contextPlatformBlock : key.entrySet()) {
                        if (contextPlatformBlock.getKey() instanceof ResourceLocation contextKey) {
                            builder.append("      - ").append(contextKey.asString()).append(": ").append(contextPlatformBlock.getValue()).append("\n");
                        }
                    }
