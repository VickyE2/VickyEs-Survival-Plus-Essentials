package org.vicky.vspe.platform.systems.dimension.terrasupporteddimensions.Generator;
...
import org.vicky.vspe.platform.systems.dimension.terrasupporteddimensions.Generator.utils.Biome.type.subEnums.*;
...
import org.vicky.vspe.platform.systems.dimension.terrasupporteddimensions.Generator.utils.Biome.type.Temperature;
...
import org.vicky.vspe.platform.systems.dimension.terrasupporteddimensions.Generator.utils.Feature.DepositableFeature;
...
import org.vicky.vspe.platform.systems.dimension.terrasupporteddimensions.Generator.utils.Meta.Configuration;
import org.vicky.vspe.platform.systems.dimension.terrasupporteddimensions.Generator.utils.Meta.HeightTemperatureRarityCalculationMethod;
...
import org.vicky.vspe.platform.systems.dimension.terrasupporteddimensions.Generator.utils.Palette.InlinePalette;
...
import org.vicky.vspe.platform.systems.dimension.terrasupporteddimensions.Generator.utils.Variant.ClimateVariant;
import org.vicky.vspe.platform.systems.dimension.terrasupporteddimensions.Generator.utils.Variant.Variant;
import org.vicky.vspe.platform.systems.dimension.terrasupporteddimensions.Generator.utils.progressbar.ProgressListener;
import org.vicky.vspe.platform.utilities.ZipUtils;
...
import static org.vicky.vspe.platform.systems.dimension.terrasupporteddimensions.Generator.utils.Utilities.getIndentedBlock;

public abstract class BaseGenerator implements Identifiable {
    public final String packID;
    public final List<BaseBiome> BIOMES;
    private final Version packVersion;
    ...
    public BaseGenerator(@NotNull String packID, Version packVersion, String packAuthors) {
    ...
    public Duration generatePack(ProgressListener progressListener) throws IOException, MissingConfigrationException {
    ...
    private void writeYamlToZip(ZipUtils zipUtils, StringBuilder content, String path, String fileName) {
        try {
            ...
            VSPEPlatformPlugin.get().getPlatformLogger().error("Failed to write to zip: " + var6, var6);
        }
    }
    ...
    private void handleFinalFileOperations(File temporaryZip) throws IOException {
        ...
        if (!temporaryZip.renameTo(renamedFile)) {
            VSPEPlatformPlugin.get().getPlatformLogger().info("Failed to rename file");
        }
        ...
        VSPEPlatformPlugin.get().getPlatformLogger().info("File moved to Terra @ " + copiedFile.getPath());
        if (renamedFile.delete()) {
            VSPEPlatformPlugin.get().getPlatformLogger().info("Temporary file deleted.");
        } else {
            VSPEPlatformPlugin.get().getPlatformLogger().info("Failed to delete the original file.");
        }
    }
    ...
    private Map<String, Map<String, StringBuilder>> getBiomesYml() {
        ...
        for (BaseBiome biome : this.BIOMES) {
            ...
            if (biomeIds.containsKey(biome.getId())) {
                VSPEPlatformPlugin.get().getPlatformLogger()
                        .warn(
                                "Biome "
                                        + biome.getClass()
            ...
            switch (biome) {
                case Variant variant -> {
                    if (variant.getVariantOf() != null) {
                        ...
                        VSPEPlatformPlugin.get().getPlatformLogger().info("Added Variant: " + variant + " to BiomeVariant: " + biomeVariant);
                    }
                    ...
                    if (variant.getClimateVariantOf() != null) {
                        ...
                        VSPEPlatformPlugin.get().getPlatformLogger().info("Added Variant: " + variant + " to ClimateVariant: " + climateVariant);
                    }
                }
                case BiomeVariant biomeVariant -> {
                    ...
                    VSPEPlatformPlugin.get().getPlatformLogger().info("Added BiomeVariant: " + variantName + " with rarity: " + selfRarity);
                }
                case ClimateVariant climateVariant -> {
                    ...
                    VSPEPlatformPlugin.get().getPlatformLogger().info("Added ClimateVariant: " + variantName + " with rarity: " + selfRarity);
                }
                default -> {
                }
            }
    ...
    private StringBuilder outputBiomeVariants() {
        ...
        if (!variantMap.isEmpty()) {
            ...
            Map.Entry<BiomeVariant, List<Variant>> firstVariant = variantList.getFirst();
