...
public class NMSChunkGenerator extends ChunkGenerator {
    ...
    public CompletableFuture<ChunkAccess> fillFromNoise(@NotNull Executor executor, @NotNull Blender blender,
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            BukkitBiome[] paddedBiomeGrid = precomputePaddedBiomeGrid(startX, startZ, seed, radius);
            ...
            for (int lx = 0; lx < 16; lx++) {
                for (int lz = 0; lz < 16; lz++) {
                    ...
                    BukkitBiome biome = paddedBiomeGrid[(lx + radius) + (lz + radius) * paddedW];
     ...
     */
    private double biomeSimilarityWeight(BukkitBiome a, BukkitBiome b) {
     ...
     */
    private BukkitBiome[] precomputePaddedBiomeGrid(int startX, int startZ, long seed, int radius) {
        int paddedW = 16 + 2 * radius;
        BukkitBiome[] grid = new BukkitBiome[paddedW * paddedW];

        int baseX = startX - radius;
        int baseZ = startZ - radius;
        NMSBiomeSource nmsSrc = (NMSBiomeSource) biomeSource;
        for (int pz = 0; pz < paddedW; pz++) {
            for (int px = 0; px < paddedW; px++) {
                int worldX = baseX + px;
                int worldZ = baseZ + pz;
                BukkitBiome b = nmsSrc.getBiomeProvider().resolveBiome(worldX, 64, worldZ, seed);
    ...
    private int[] computeBiomeAwareBlendedHeightsOptimized(int chunkX, int chunkZ, int startX, int startZ,
                                                           BukkitBiome[] paddedBiomeGrid, int radius) {
        final int W = 16, H = 16;
        int paddedW = 16 + 2 * radius;
        int[] out = new int[W * H];
        ...
        for (int lz = 0; lz < H; lz++) {
            for (int lx = 0; lx < W; lx++) {
                ...
                int worldX = startX + lx;
                int worldZ = startZ + lz;
                BukkitBiome centerBiome = paddedBiomeGrid[(lx + radius) + (lz + radius) * paddedW];
                ...
                for (int sdz = -radius; sdz <= radius; sdz++) {
                    for (int sdx = -radius; sdx <= radius; sdx++, kIdx++) {
                        ...
                        BukkitBiome sampleBiome = paddedBiomeGrid[sampleLocalX + sampleLocalZ * paddedW];
